defmodule Collabiq.Proxy do
  ...
  def create(attrs, sess) do
    ...
    with :ok <- Security.validate_perms(%{admin: [:create_proxy]}, sess),
  ...
  def get(id, sess) do
    with :ok <- Security.validate_perms(%{admin: [:create_proxy, :manage_proxy, :purge_proxy]}, sess),
         {:ok, struct} <-
           __MODULE__
           |> where([q], q.id == ^id)
  ...
  defp modify(attrs, sess) do
    ...
    with :ok <- Security.validate_perms(%{admin: [:manage_proxy]}, sess),
  ...
  def purge(id, sess) do
    with :ok <- Security.validate_perms(%{admin: [:purge_proxy]}, sess),
         {:ok, struct} <- get(id, sess),
         change <- cs(struct, %{}),
         :ok <- Repo.validate_change(change),
